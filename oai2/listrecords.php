...
if (isset($args['resumptionToken'])) {
    if (count($args) > 1) { // overwrite all other errors
        ...
        $options['offset'] = $OAI_CONFIG['MAXRECORDS'] * $args['page'];
        $page = $args['page'];
        unset($args['page']);
    }
} else { // no, new session
...
foreach ($records as $record) {
    ...
    $output .='>'."\n";
    $output .= xmlformat($identifier, 'identifier', '', 5);
