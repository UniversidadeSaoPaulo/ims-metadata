 ...
 */
//
// $Id: listsets.php,v 1.2 2010/05/13 15:13:45 jmg324 Exp $
//
...
if (is_array($OAI_CONFIG['SETS'])) {
    $output .= " <ListSets>\n";
    foreach ($OAI_CONFIG['SETS'] as $key=>$val) {
        $output .= " <set>\n";
        ...
        if (isset($val['setDescription']) && $val['setDescription'] != '') {
            ...
            $output .= metadataHeader('oai_dc');
            $output .= xmlrecord($val['setDescription'], 'dc:description', '', 7);
            $output .= ' </oai_dc';
            if (isset($OAI_CONFIG['METADATAFORMATS']['oai_dc']['record_prefix'])) {
                $output .= ':'.$OAI_CONFIG['METADATAFORMATS']['oai_dc']['record_prefix'];
            }
            $output .= ">\n";
            $output .= " </setDescription>\n";
        }
